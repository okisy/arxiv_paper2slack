...
GitHub ActionsのCIと同じ検証（静的解析およびユニットテスト）をローカルで簡単に実行できるよう、リポジトリルートに `test.sh` および `Makefile` を用意しています。CIエラーを未然に防ぐため、コミット前にテストを行ってください。

### テストと静的解析の実行（推奨）

ルートディレクトリで以下のコマンドを使用して、静的解析 (ruff) と テスト (pytest) を一括実行します。
